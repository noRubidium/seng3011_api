...
class RemoteResponse(object):
    ...
    def remote_get(remote_type, categories, states, starting_date, ending_date):
        ...
        try:
            ...
            if remote_type == 'merch':
                f = open('M.1.3.expected.json')
                RemoteResponse.total_dict['merch'] = disassemble_json(json.loads(f.read())['MonthlyCommodityExportData'], 'commodity',
                                                                  'value')
                f.close()
            else:
                f = open('R.1.3.expected.json')
                RemoteResponse.total_dict['retail'] = disassemble_json(json.loads(f.read())['MonthlyRetailData'], 'category',
                                                                   'turnover')
